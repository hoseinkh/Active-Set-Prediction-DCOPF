...
num_of_top_classes_to_consider = 74   # avoid training on less populated classes (improve speed)
...
if use_weighted_classififcation:
    ...
    clf = svm.SVC(kernel='linear', C=0.00001, class_weight=class_weights_dict)
    # Train classifier
    clf.fit(X_train_trimmed, y_train_trimmed)
else:
    # Create a linear SVM classifier
    clf = svm.SVC(kernel='linear', C=10, probability=True, break_ties=True, verbose=True)
    # Train classifier
    clf.fit(X_train_trimmed, y_train_trimmed)
#
#
