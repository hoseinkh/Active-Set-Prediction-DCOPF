...
dict_of_samples = {}
for i in set(y_train_trimmed.tolist()):
    curr_num_samples_per_curr_class = sum(y_train_trimmed == i)
    if curr_num_samples_per_curr_class < 100:
        dict_of_samples[i] = 100
    else:
        dict_of_samples[i] = curr_num_samples_per_curr_class
oversample = RandomOverSampler(sampling_strategy=dict_of_samples)
# oversample = RandomOverSampler(sampling_strategy='minority')
#
## fit and apply the transform
X_train_trimmed_over, y_train_trimmed_over = oversample.fit_resample(X_train_trimmed, y_train_trimmed)
#
# # Create LDA
...
# lda = LDA(solver='lsqr', tol = 0.000001, shrinkage=0.13)
##
# lda = LDA(solver='lsqr', tol = 0.000001)
# oa = OAS(store_precision=False, assume_centered=False)
...
#
# # Train classifier
lda_history = lda.fit(X_train_trimmed_over, y_train_trimmed_over)
# lda_history = lda.fit(X_train_trimmed, y_train_trimmed)
#
