...
%
% This code generates 55276 feasible cases
%
...
num_of_target_feasible_cases = 55276;
num_RPPs = 37;
...
%
% curr_dir = pwd; 
% parent_dir = '/Users/h.khazaei/AppsH/PycharmProjects/Cong_Pred__Modified_Power_Sys_New/Codes'; % This is for old Mac!  
% 
parent_dir = '/Users/hosseinkhazaei/AppsH/PycharmProjects/Cong_Pred__Modified_Power_Sys_New/Codes';
...
%
% mpc.H_list_of_buses_with_RPP = [62,124,126,17,125,72,8,14,27,30,103,60,   140,  150,  160];
% mpc.H_list_of_buses_with_RPP = [125, 62, 72, 8, 124, 14, 103, 3, 27, 60, 126, 10, 52, 147, 13, 12, 123, 94, 132, 98, 148, 96, 30, 95, 93, 117, 50, 59, 93, 117, 50, 59, 15, 78, 17, 31, 54];
% mpc.H_list_of_buses_with_RPP = [125, 72, 8, 14, 103, 3, 27, 60, 10, 52, 147, 13, 12, 123, 94, 132, 98, 148, 96, 30, 95, 93, 117, 50, 59, 93, 117, 50, 59, 78, 31, 54];
%
rng(65,'twister');
s = rng;
rng(s);
% mpc.H_list_of_buses_with_RPP = randi(162,1,num_RPPs);
mpc.H_list_of_buses_with_RPP = [5,6,9,17,17,18,18,18,22,27,27,28,30,36,36,38,39,47,50,57,73,74,79,82,84,87,90,91,103,108,111,114,120,125,129,151,160];
%
mpc.H_list_of_buses_without_RPP = setdiff([1:num_nodes],mpc.H_list_of_buses_with_RPP);
%
% to increase the number of feasible cases
% original_loads_at__buses_with_RPP = 1.5*(mpc.bus(mpc.H_list_of_buses_with_RPP,3));
%
% % % Here we add a Normal noise to the loads specified in mpc.H_list_of_buses_with_RPP 
% mpc.H_mean_of_RPPS = 1.5*abs(mpc.bus(mpc.H_list_of_buses_with_RPP,3));
coeff_RPP_1 = 0.40;
coeff_RPP_2 = 0.30;
mpc.H_mean_of_RPPS = coeff_RPP_1*sum(mpc.bus(:,3))/num_RPPs * ones(1,num_RPPs);
mpc.H_std_of_RPPs  = coeff_RPP_2 .* mpc.H_mean_of_RPPS';
% mpc.H_mean_of_RPPS = coeff_1*mpc.bus(mpc.H_list_of_buses_with_RPP,3);
% mpc.H_std_of_RPPs  = coeff_2 .* mpc.H_mean_of_RPPS';
% we assume that the noises are independent
mpc.H_cov_matrix_of_RPPs = diag(mpc.H_std_of_RPPs)*diag(mpc.H_std_of_RPPs);
%
%
original_loads_at__buses_with_RPP = mpc.bus(mpc.H_list_of_buses_with_RPP,3);
%% step 4: generate samples
%
% The following command is for the uniform pdf of the noises!
% generated_samples___ALL = zeros(num_of_samples,length(mpc.H_list_of_buses_with_RPP));
% for i = 1:length(mpc.H_list_of_buses_with_RPP)
...
if random_load_on_all_nodes
    ...
    generated_samples___ALL(:,mpc.H_list_of_buses_with_RPP) = generated_samples___ALL(:,mpc.H_list_of_buses_with_RPP) - mvnrnd(mpc.H_mean_of_RPPS,mpc.H_cov_matrix_of_RPPs,num_of_samples);
elseif random_load_on_nodes_without_RPP_only
    coeff_load_1 = 1;
    coeff_load_2 = 0.06;
    mpc.H_mean_of_loads = coeff_load_1*mpc.bus(mpc.H_list_of_buses_without_RPP,3) + 0.000000001;
    mpc.H_std_of_loads  = coeff_load_2 .* abs(mpc.H_mean_of_loads');
    % this part I added for limiting the variance
    if false
        mpc.H_std_of_loads = [];
    ...
    generated_samples___ALL(:,mpc.H_list_of_buses_with_RPP) = repmat(original_loads_at__buses_with_RPP',num_of_samples,1) - mvnrnd(mpc.H_mean_of_RPPS,mpc.H_cov_matrix_of_RPPs,num_of_samples);
    generated_samples___ALL(:,mpc.H_list_of_buses_without_RPP) = mvnrnd(mpc.H_mean_of_loads',mpc.H_cov_matrix_of_loads,num_of_samples);
else
end
%
...
curr_num_feasible_samples = 0; % 
%
for i = 1:1:num_of_samples
    % set the demand at the buses that we generated sample for
%     mpc.bus(mpc.H_list_of_buses_with_RPP,3) = original_loads_at__buses_with_RPP - generated_samples___ALL(i,:)';
    % for measuring time
    tic
    %
    ...
    if results.success == 0 % infeasible solution
        ...
        curr_num_feasible_samples = curr_num_feasible_samples + 1;
        if curr_num_feasible_samples > num_of_target_feasible_cases
            break
        end
        %
        toc
