...
num_of_top_classes_to_consider = 319   # avoid training on less populated classes (improve speed)
...
#
# NN
num_features = len(X_train[0, :].tolist())
...
#
# NN needs one-hot encoding
set_of_classes_in_train_trimmed = set(y_train_trimmed)
set_of_classes_in_test_trimmed = set(y_test_trimmed)
set_of_classes_in_all_trimmed = set()
set_of_classes_in_all_trimmed = set_of_classes_in_all_trimmed.union(set_of_classes_in_train_trimmed)
set_of_classes_in_all_trimmed = set_of_classes_in_all_trimmed.union(set_of_classes_in_test_trimmed)
num_of_classes_in_trimmed = len(set_of_classes_in_all_trimmed)
#
y_train_trimmed_onehot_encoded = []
for i in y_train_trimmed.tolist():
    new_label_to_be_added = [0 for j in range(0,num_of_classes_in_trimmed)]
    new_label_to_be_added[i-1] = 1
    y_train_trimmed_onehot_encoded.append(new_label_to_be_added)
y_train_trimmed_onehot_encoded = np.array(y_train_trimmed_onehot_encoded)
#
y_test_trimmed_onehot_encoded = []
for i in y_test_trimmed.tolist():
    new_label_to_be_added = [0 for j in range(0,num_of_classes_in_trimmed)]
    new_label_to_be_added[i-1] = 1
    y_test_trimmed_onehot_encoded.append(new_label_to_be_added)
y_test_trimmed_onehot_encoded = np.array(y_test_trimmed_onehot_encoded)
#
num_of_distinctive_classes_in_trimed_version = len(set_of_accepted_classes)
#
#
#
#
...
#
model = Sequential([
    Dense(5*num_features, activation='relu', input_dim=num_features),
    # Flatten(),
    # Dense(3*num_features, activation='tanh'),
    Dense(5*num_features, activation='relu', kernel_regularizer=l2(0.1)),
    # Dropout(0.2),
    # Dense(4*num_features, activation='sigmoid'),
    # Dense(1*num_features, activation='relu'),
    Dense(num_of_distinctive_classes_in_trimed_version, activation='softmax'),
    # Dense(num_of_distinctive_classes_in_trimed_version, activation='sigmoid'),
    # Dense(num_of_distinctive_classes_in_trimed_version, activation='tanh'),
...
tensorflow.keras.optimizers.Adam(learning_rate=0.003)
# opt = SGD(lr=0.003)
#
...
#
# Training the model.
history = model.fit(X_train_trimmed, y_train_trimmed_onehot_encoded, epochs=1000, validation_data=(X_test_trimmed, y_test_trimmed_onehot_encoded), shuffle=False, verbose = 1, batch_size = 50)
#
#
...
overall_scores = model.predict(X_test_trimmed)
max_score___multi_candidate = []
new_Hmatpower_object = Hmatpower(case_number="ClassHmatpower_for_IEEE_162_bus",
                                 gen_DA_RT_list=["D" for i in range(0, 12)], epsH=0.0001,
                                 probability_threshold_r1=0.9, probability_threshold_r2=0.9,
                                 probability_threshold_r3=0.9, ratio_std_to_mean_r1=0.2, ratio_std_to_mean_r2=0.2,
                                 ratio_std_to_mean_r3=0.2)
...
if use_multi_candidate_scroing_approach:
    ...
    for i in range(0, len(y_test_trimmed)):
        ...
        true_class_trimmed = y_test_trimmed[i] - 1
        if len(set_of_accepted_classes) == 2:
            ...
            if true_class_trimmed in max_score___multi_candidate[i, :].tolist():
                ...
                true_class_trimmed___original_label = dic_mapping_new_labels_to_original_labels[true_class_trimmed + 1]
...
#
#
# np.array([list_of_num_of_samples_not_corectly_classified___for_each_class_in_testing, array_of_num_of_patterns_for_each_dist_cong_pttrn_in_testing, array_of_num_of_patterns_for_each_dist_cong_pttrn_in_training])
# np.array([list_of_num_of_samples_not_corectly_classified___for_each_class_in_testing, array_of_num_of_patterns_for_each_dist_cong_pttrn_in_testing, array_of_num_of_patterns_for_each_dist_cong_pttrn_in_training, list_of_percentage_samples_not_corectly_classified___for_each_class_in_testing])
gggggg = 5+6