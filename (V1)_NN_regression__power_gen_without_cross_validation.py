...
for i in range(0,num_of_distinctive_classes):
  ...
  if curr_num_of_patterns_for_curr_dist_cong_pttrn >= 4000: # we have enough samples for training
    counter_model += 1
    ...
    #
    #
    len_of_input_array_to_the_NN  = len(samples_for_current_model_trarin_and_test[0])
    len_of_output_array_of_the_NN = len(labels_for_current_model_trarin_and_test[0])
    #
    #
    ...
    #
    # Training the model.
    exec("model" + str(counter_model) + ".fit(samples_for_current_model_trarin_and_test, labels_for_current_model_trarin_and_test, epochs=4000, validation_split = 0.20, shuffle=True, verbose = 2, batch_size = 128)")
    #
